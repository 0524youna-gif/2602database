...
| 본문의 핵심 | 나의 설명 |
| --- | --- |
| AI가 SQL을 만들어도 DB를 배워야 한다 | 사람이 항상 결과값에 책임져야 하고 AI가 매순간 정확한 것이 아니기 때문에 DB를 배울 필요가 있다.  |
| 데이터 구조가 중요하다 | 데이터를 어떤 기준으로 나누고 연결하는지에 따라 필요한 정보를 찾고 분석하는 방식이 달라진다.  |
| SQL 결과를 검증해야 한다 | 같은 결과가 나와도 조건이나 계산 과정이 잘못되었을 수 있으므로, 원하는 데이터가 맞는지 직접 확인해야 한다.   |
| 데이터 품질이 AI 답변 품질에 영향을 준다 | 데이터에 오류가 있거나 정보가 부족하면 AI도 정확한 답을 만들기 어렵다. |
| 저장 방식은 데이터 양만 보고 선택하지 않는다 | 데이터를 어떻게 사용하고, 얼마나 자주 수정하거나 빠르게 찾아야 하는지도 함께 고려해야 한다. |

## 가장 중요하다고 생각한 한 문장

```text 사람은 데이터의 의미와 결과를 확인해야 한다
